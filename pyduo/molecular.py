...
class MolecularSystem:
    ...
    def single_fit_step(self, state, component_name, verbose=False, state_fit=False):
        ...
        def callback(params):
            ...
            # try:
            #     if state.name=='B2Sig+':
            #         self.states['A2Pi'].potential_energy.values_dict['DE']=self.states['B2Sig+'].potential_energy.values_dict['DE']
            # except:
            #     pass
    ...
    def single_fit_step_off_diag(self, component, verbose=False, state_fit=False):
        """
        Performs a single fit step for the specified component.
        ...
        def objective_function(params, component):
            ...
            if state_fit:
                self.run_duo(component.state_numbers)
            # Run DUO and generate output
            else:
                self.run_duo()
            # Calculate the RMS error from DUO output
    ...
    def iterative_fit(self, state_vary=None, off_diagonal_vary=None, convergence_threshold=1e-6, max_iterations=100, verbose=False, state_fit=False, vary_diag_first=False):
        """
            ...
            if vary_diag_first:
                if off_diagonal_vary:
                    for off_diagonal_name in off_diagonal_vary.keys():
                        ...
                        self.single_fit_step_off_diag(self.off_diagonal[off_diagonal_name],state_fit=state_fit)
                        self.run_duo()
