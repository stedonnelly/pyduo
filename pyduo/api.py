...
class duo_output_object:
            ...
            try:
                if state_number:
                    if type(state_number)==int:
                        ...
                        self.rms = self.weighted_rms_states(state_number)
                else:
    ...
    def weighted_rms_states(self, state_numbers):
        """
        Calculate the weighted root mean square error between observed and calculated values.
        """
        filtered_df = self.output[self.output['States1'].isin(state_numbers)]
        obs = filtered_df['obs']
        calc = filtered_df['calc']
        squared_errors = (obs - calc) ** 2
        weighted_squared_errors = squared_errors * filtered_df['Weight']
...
def process_blocks(input_file):
    ...
    for line in file_lines:
        ...
        if words[0].lower() in block_keywords:
            ...
            elif key.startswith('B') and key != 'BINF' and key != 'BETA':
...
def plot_residuals(output_filepath=None, rot_state=False, energy=False, log=False, lambda_rep=False, save=False):
    ...
    for state_idx, (state, state_group) in enumerate(df_new_filtered_with_J.groupby('state')):
        ...
        for vib_idx, (vib, group_data) in enumerate(state_group.groupby('vib')):
            ...
            epp1 = group_data_p_plus['Calc']
            emp1 = group_data_p_minus['Calc']
            if log:
                group_data_p_plus['obs-calc'] = np.log10(abs(group_data_p_plus['obs-calc']))
                group_data_p_minus['obs-calc'] = np.log10(abs(group_data_p_minus['obs-calc']))
            ...
            if rot_state:
                ...
                if state==1:
                    ax.scatter(epp1, group_data_p_plus['obs-calc'], color=vib_colors[vib_idx], marker='+', label=f'vib={vib}, p=+', s=90)
                    ax.scatter(emp1, group_data_p_minus['obs-calc'], color=vib_colors[vib_idx], marker='_', label=f'vib={vib}, p=-', s=90)
                elif state==2:
                    ax.scatter(epp1, group_data_p_plus['obs-calc'], color=vib_colors[vib_idx], marker='+', label=f'vib={vib}, p=+', s=90)
                    ax.scatter(emp1, group_data_p_minus['obs-calc'], color=vib_colors[vib_idx], marker='_', label=f'vib={vib}, p=-', s=90)
                elif state==3:
        ...
        ax.set_ylabel('log$_{10}($Obs-Calc.$)$, cm$^{-1}$',fontsize=12)
    ...
    plt.show()
    if save:
        name = output_filepath.replace('en','png')
        fig.savefig(name)
        plt.close()
...
def plot_abinitio_calc(data_dict):
    ...
    if num_states != 0:
        ...
        raise ValueError('Number of states is 0')
...
def run_duo_input(input_filename):
    output_filename = 'out.'+input_filename.split('.')[0]
    command = f"duo < {input_filename} > {output_filename}"
